...
def the_predict(model, sentences):
    id2word, corpus = load_dic_mm_file()
    # 문장을 형태소 단위로 분리
    m = Mecab()
    key = m.pos(sentences)
    print(key)
...
def load_dic_mm_file():
    if (os.path.exists(os.environ['CHATBOT_ROOT']+"/resources/weights/THE_weights/The_model(LDA).dict")):
        id2word = corpora.Dictionary.load(os.environ['CHATBOT_ROOT']
                                               + "/resources/weights/THE_weights/The_model(LDA).dict")
        corpus = corpora.MmCorpus(os.environ['CHATBOT_ROOT']+"/resources/weights/THE_weights/The_model(LDA).mm")
